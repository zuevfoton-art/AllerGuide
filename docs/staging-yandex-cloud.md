...
| `mobile-android` / **EAS staging Android** / Free plan this month | EAS quota. Upload мог пройти, build — нет. Не откатывать API (`smoke` уже зелёный). APK: `staging-apk-gradle.yml`. После 1-го числа / paid plan — снова EAS |
