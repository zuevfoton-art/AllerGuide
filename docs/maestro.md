...
| Offline nightly: Maven 403 `gson-2.9.1.pom` / `Error resolving plugin com.facebook.react.settings` (staging при этом собрался) | Nightly **35198863494**: `repo.maven.apache.org` отдал 403 на preview. `maestro-build-apk.sh` повторяет `assembleRelease` до 3 раз |
