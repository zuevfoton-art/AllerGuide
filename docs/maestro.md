...
| `"зуд"` не видно в `diary-field-symptoms` | Текст уже в поле, но 18 чипов каталога выше него; Yoga инвертирует bounds (`[87,1807][993,1395]`, nightly 34477934128). Обязательное текстовое поле идёт первым на grouped-экране; `_fill-wizard-field` после IME снова `scrollUntilVisible` перед `assertVisible` |
