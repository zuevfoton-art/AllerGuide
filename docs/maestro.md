...
| Offline nightly: `:app:mergeDexRelease` / `Java heap space` (staging при этом зелёный) | Gradle daemon был `-Xmx2048m` и предупреждал, что metaspace 512 MiB исчерпан. Nightly 34474685308 собрал JS, потом OOM на D8. `maestro-build-apk.sh` после `expo prebuild` пинит `-Xmx4096m -XX:MaxMetaspaceSize=1024m` и зовёт `assembleRelease --no-parallel` |
