...
| Offline nightly: `No visible element found: id: diary-choice-Слабый` (чип в дампе, Gboard открыта) | Nightly **34946086211**: `diary-choice-Слабый` `[232,1925][428,2024]`, `Далее` disabled, appearance=шелушение. Тап `diary-editor-title` не снял IME у `Modal` (чужой window token). `_dismiss-wizard-ime` тапает title, затем `diary-wizard-step-label`; title/`step-label` зовут `blurDiaryEditorIme` на `onPressIn`; `_tap-wizard-choice` ждёт `visible` чипа |
| Offline nightly: `Assertion is false: id: diary-choice-Слабый is visible` (чип в дампе, Gboard открыта) | Nightly **35067465304** на SHA с фиксом 34946086211: те же bounds `[232,1925][428,2024]`, `Далее` disabled, appearance=шелушение. `Keyboard.dismiss` в `Modal` — no-op; после fill `focusedInputRef` обнуляли, второй blur не к чему. Компактные choice-чипы (зуд) пинятся в `diary-editor-pinned-top` над полями; `blurDiaryEditorIme` зовёт host `.blur()` и не чистит ref; тап чипа тоже `dismissIme` |
