...
| `profile-delete` не найден в `sos-no-profile-smoke` | Удаление на `/profile-edit`, не на хабе. Тап `profile-list-item-0`, ждать `profile-edit-title`, `scrollUntilVisible` → `profile-delete` |
