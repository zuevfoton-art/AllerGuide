...
| Offline nightly: `No visible element found: id: diary-choice-Слабый` (чип в дампе, Gboard открыта) | Nightly **34946086211** / **34956812041**: чип `[232,1925][428,2024]` под Gboard, `Далее` disabled. Maestro `inputText` не шлёт RN `onFocus`, поэтому blur по «последнему» ref был no-op. Регистрировать все TextInput на mount; `blurDiaryEditorIme` блюрит все; `_tap-wizard-choice` сначала `scrollUntilVisible`, потом `visible` |
