...
| Offline nightly: Maven 403 `gson-2.9.1.pom` / `Error resolving plugin com.facebook.react.settings` (staging при этом собрался) | Nightly **35198863494**: `repo.maven.apache.org` отдал 403 на preview. `maestro-build-apk.sh` повторяет `assembleRelease` до 3 раз, а `setup-java` в обоих джобах держит `cache: gradle`, чтобы неизменная сборка вообще не ходила в сеть |
| После локального прогона `maestro-build-apk.sh` дерево грязное | Скрипт зовёт `expo prebuild` и патчит манифест под cleartext, а `apps/mobile/android/` закоммичен: меняются `build.gradle`, `AndroidManifest.xml`, иконки, плюс появляется `res/xml/network_security_config.xml`. На эфемерном раннере это неважно, локально — `git checkout -- apps/mobile/android` перед коммитом |
| APK собран не с тем профилем (в preview зашиты staging API URL и DSN) | Ни одна `EXPO_PUBLIC_*` не входит во входы задачи бандлинга, поэтому после сборки другого профиля Gradle пишет `:app:createBundleReleaseJsAndAssets UP-TO-DATE` и переиспользует прежний JS. Замер локально: `staging`, затем `preview` дали побайтово одинаковые бандлы. `maestro-build-apk.sh` удаляет `app/build/generated/assets/react/release` перед сборкой |
| `The Daemon will expire after the build after running out of JVM Metaspace` | Поднимать metaspace только в `gradle.properties` бесполезно: `maestro-build-apk.sh` передаёт `-Dorg.gradle.jvmargs` через CLI, а CLI приоритетнее файла. Оба места держат `-Xmx4096m -XX:MaxMetaspaceSize=1024m`; инвариант в `scripts/maestro-ci-check.test.mjs` сверяет их посимвольно |
...
| `profile-screen-title` не найден при открытой IME | Phone-pad заставляет Android доскроллить хаб к полю ввода (замер: 220 dp при окне 533 dp), а заголовок жил в ScrollView и уезжал за верхний край. UiAutomator отдаёт такой узел с верхом, прижатым к кромке скролла, и низом выше него: nightly 34340207243 — `[190,380][892,357]`, высота −23. `ScreenHeader` перенесён в `pinnedTop`; общее правило — anchor, по которому тапают **после** прокрутки, должен быть в закреплённой обвязке, а не в children |
| `profile-header-button` не найден на «Сегодня» | Nightly **35198863494**: Earth Wellness chrome рендерит именной чип (`Maestro Staging`), а не иконку. Чип жил с `profile-header-chip`, staging-auth/settings/SOS ждали `profile-header-button`. Оба варианта триггера несут `profile-header-button` |
| `hint-overlay` / `hint-skip` не появились, «Сегодня» стоит на скелетонах | Nightly **35315005471** (оба джоба): загрузчик главной ждёт GPS-фикс, а у `getCurrentPositionAsync` нет своего дедлайна — на AVD без подставленной локации он не разрешается никогда, `loadingWellness` остаётся `true`, и `useHintTour('home', { ready: !loadingWellness })` не стартует. Фикс — гонка с `POSITION_TIMEOUT_MS` (8 с) и откат на регион по умолчанию; ожидания оверлея в flow подняты до 40 с (8 с GPS + 8 с Open-Meteo) |
