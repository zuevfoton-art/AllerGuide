...
| `profile-save-number` не найден после ввода 112 | Phone-pad на Pixel 6 выкидывает кнопку из UiAutomator (nightly 34052584781). `_dismiss-profile-ime.yaml` сворачивает IME тапом инертного текста, затем `scrollUntilVisible` → `profile-save-number` |
| `profile-screen-title` не найден при сворачивании IME на «Мои профили» | Фокус в поле номера прокручивает хаб: шапка уходит выше вьюпорта ScrollView, и UiAutomator обрезает её bounds по верхней границе — высота отрицательная, тап невозможен (nightly 34340207243: заголовок `[190,380][892,357]`, `screen-header-back` `[53,380][158,370]`). Якорь сворачивания IME должен быть рядом с полем: `_dismiss-profile-ime.yaml` тапает `profile-emergency-hint` (в дампе `[92,1459][990,1507]`) |
