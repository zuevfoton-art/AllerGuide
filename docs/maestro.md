...
| `settings-smoke` | номер SOS: ввод → `_dismiss-profile-ime` → `profile-save-number` |
...
| `profile-save-number` не найден после ввода 112 | Numeric keypad перекрывает «Сохранить номер». `_dismiss-profile-ime.yaml` (тап `profile-screen-title`), затем `scrollUntilVisible` (nightly 34052584781) |
