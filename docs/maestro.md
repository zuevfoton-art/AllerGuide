...
| `staging-backup-smoke` | recovery key fixture → upload backup → banner «Резервная копия отправлена на сервер.» |
...
| `diary-photo-step` не появился после «Слабый» | Gboard не закрылся (title был View, не Pressable), второе поле не взяло фокус: `skinArea=лицоyпокраснение`, appearance пустое, Далее disabled (nightly 34349155324). Шапка вызывает `dismissDiaryKeyboard` (`blurTextInput` + `Keyboard.dismiss`); поля в обёртке фиксированной высоты; `_fill-wizard-field` тапает поле повторно после layout; `_tap-wizard-primary` требует `enabled: true` |
| `diary-wizard-primary` Enabled не найден на Коже | Nightly 34451477109: `Keyboard.dismiss` не снимал фокус, `лицоyyпокраснение` в skinArea, Далее `enabled=false`. Нужен `TextInput.State.blurTextInput` |
| Staging `«Готово» is visible` | `CloudBackupCard` больше не показывает Alert, а banner `uploadSuccess`. `staging-backup-smoke` ждёт `status-banner` |
