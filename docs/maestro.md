...
| Offline nightly: `"шелушение"` не видно в `diary-field-appearance` | Nightly **35072335460**: `skinArea=предплечьеyyшелушение`, appearance остался плейсхолдером, Далее disabled. Тап appearance попал в Gboard (`[87,1552][993,1867]`), ввод ушёл в первое поле. `splitDiaryScreenForIme` пинит `skinArea` + `appearance` + зуд в `diary-editor-pinned-top` в порядке схемы; пустой scroll не рендерит карточку; `_fill-wizard-field` ждёт `visible` FIELD_ID перед вводом |
| Offline nightly: `Element not found: diary-field-appearance` (поле в дампе, Gboard открыта) | Nightly **35094037122** на SHA с пином 35072335460: `предплечье` в зоне, appearance focused на плейсхолдере `[42,1558][1038,1873]` под Gboard, Далее disabled. Первый tapOn сработал, второй — нет. Пока текстовое поле в фокусе, в chrome остаётся только оно (tap-min высота); `_fill-wizard-field` печатает после первого тапа, второй `optional: true`. Title/`dismissIme` снова показывает все шаги |
