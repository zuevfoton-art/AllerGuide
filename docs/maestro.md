...
| Offline nightly: `assertVisible "зуд"` на `diary-field-symptoms` | Чипы справочника + multiline + голос ниже fold. Nightly 34477934128: текст `зуд` записан, bounds `[87,1807][993,1395]` инвертированы. `_fill-wizard-field` после IME dismiss делает `scrollUntilVisible` (как `_tap-wizard-primary`) и только потом `assertVisible` |
