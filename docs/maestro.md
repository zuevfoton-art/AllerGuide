...
| `staging-backup-smoke` | recovery key fixture → upload backup → `status-banner` |
...
| `Готово` не видно после upload бэкапа | Alert заменили на `StatusBanner` (`uploadSuccess`, не title «Готово»). Nightly 34451477109 ждал текст «Готово» 60 с. `staging-backup-smoke` ждёт `status-banner` |
