...
| Offline smoke: `Build preview APK` / `:app:mergeDexRelease` / `Java heap space` | Nightly 34474685308: Gradle JVM был `-Xmx2048m`. Staging-job на том же SHA собрался, preview — нет. `gradle.properties` и `maestro-build-apk.sh` (`pin_gradle_heap` после `expo prebuild`) держат `-Xmx4096m` |
