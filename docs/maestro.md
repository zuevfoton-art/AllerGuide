...
| `diary-wizard-primary` не виден сразу после «Симптомы» | Каталог чипов + голос выталкивают «Далее» за `maxHeight` шита (nightly 34325395361, bounds `[87,2373][993,2358]`). Действия в `DiaryEditorFooter` вне ScrollView; `_wait-wizard-primary.yaml` ждёт `diary-wizard-step-label` и `scrollUntilVisible` |
