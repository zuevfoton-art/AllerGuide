...
| `diary-wizard-primary is visible` падает сразу после открытия визарда | «Далее» — последний ребёнок ScrollView визарда и заблокирован, пока шаг пуст, поэтому на Pixel 6 он ложится за сгибом (nightly 34325395361: bounds `[87,2373][993,2358]`, `enabled=false`). Факт открытия визарда проверяем по `diary-wizard-step-label`; сам тап — только через `_tap-wizard-primary.yaml` |
