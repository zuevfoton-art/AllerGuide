...
**Иерархия заголовков UI (не смешивать):**

| Уровень | Компонент / стиль | Типографика | Назначение |
|---------|-------------------|-------------|------------|
| Экран | `ScreenHeader` / `ui.docTitle` | 26 / 700 | Заголовок экрана |
| Карточка | `CardTitle` / `ui.sectionTitle` | 18 / 600 serif | Заголовок карточки |
| Группа | `ui.sectionLabel` | 11 uppercase | Подпись группы карточек, **никогда** заголовок карточки |
| `src/i18n/*` | Строки и rich content. Новые ключи — во **все 6** локалей + `types.ts`. |

**Геометрия (ACTION / STATE):** радиус только через `radii` в `layout.ts`, вертикальный ритм только через `density`. `radii.full` — только на элементах, которые **запускают действие** (`Button`, поиск области, звонок 103, сегмент периода как группа). Чипы, фильтры, табы и поля держат состояние и остаются на `radii.sm` / `radii.md`. Не ставить `radii.full` на чип. Высоты тапа (`density.tapMinHeight` 44 / `tapMinHeightSm` 36) не уменьшать: плотность берём из зазоров и внутренних отступов; где визуал ниже нормы — `hitSlop`.
