...
[`brand-claro-green.md`](./brand-claro-green.md) §Правила #2 предписывает soft wellness surfaces через `GlassCard variant="soft"`. Фаза **W2** вернула `variant?: 'default' | 'soft'` (soft = `accentLight` фон + `accentMid` бордер) на wellness-карточках Главной.
...
|--------------------|-------------|------|--------|
| Тёмная тема как забота о зрении | Закрыто. Расширить контраст-гейт | W2 | done |
| Оптимальная типографика | Нет `lineHeight`/`tracking`; ~449 литеральных `fontSize`; сироты 14/16 px | W1 | done |
| Природные, ненавязчивые цвета | Холодные синие фоны (`bg #F4F6F9`, dark `#0B1120`) | W2 | done |
| Продуманные микро-взаимодействия | Нет токенов движения; reduce-motion только в Skeleton и pollen plume | W3 | done |
| Ограниченное число поп-апов | 26 `Alert.alert`; `UndoBanner` в одном месте | W4 | done |
| Гибкость и персонализация | Нет размера текста и переключателя анимаций | W5 | done |
| Минимум визуального шума | Карта: ~390–430 pt хрома (E14); Сканер: до 17 блоков результата | W6 | done |
| Баланс текста и визуала | `AsitCourseCard` не смонтирован; шкалы — текст без тренда; факторы Главной без шкалы | W8 | done |
| Единые заголовки и навигация | 9 `CardTitle` vs 38–61 ad-hoc; `ui.cardTitle` (12 px) как H2; 6 систем заголовков | W9 | done |
| Бережный возврат после паузы | Нет детектора 3+ дней; нет чек-ина в один тап | W7 | done |
