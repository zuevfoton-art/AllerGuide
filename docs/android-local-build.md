...
> **Stage = YC only** ([`migrate-off-replit-to-yc.md`](./migrate-off-replit-to-yc.md) Phase 2). Replit (`aller-guide.replit.app`) не использовать для новых сборок; EAS profile `replit` удаляется в Phase 3.
