...
- Версии в проекте: **Node 22.14.0**, **Gradle 9.0.0**, **Expo SDK 55 / React Native 0.83**, **JDK 17**.
- New Architecture: **обязательна** на SDK 55 (Legacy Arch удалён).
...
Android Gradle Plugin для RN 0.83 требует **JDK 17**.
...
> Первая сборка качает Gradle 9.0.0 и зависимости — это долго. Последующие сборки кешируются.
...
| Долгая первая сборка / таймаут скачивания Gradle | Это нормально (качается Gradle 9.0.0 + зависимости); повторный запуск использует кеш |
