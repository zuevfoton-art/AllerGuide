...
**Карта пыления (Яндекс + геолокация):** план интеграции берёзы / злаковых / амброзии — [`docs/yandex-pollen-map-integration.md`](./yandex-pollen-map-integration.md). Замена basemap на Google + `heatmapTiles` (слой «Пыление», бейдж Open-Meteo) — §4.5–4.6 того же документа.
