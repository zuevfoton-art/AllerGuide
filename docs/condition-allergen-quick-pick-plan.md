...
| Состояние | Поведение |
|-----------|-----------|
| Типы выбраны | Группы рекомендаций; кнопка каталога подписана как «Все аллергены» |
...
| Ключ | RU |
|------|-----|
| `allergens.recommendedTitle` | Рекомендуем для выбранных типов |
| `allergens.recommendedHint` | Отметьте свои. Остальные — в полном списке. |
| `allergens.recommendedGroup` | «{{label}}» |
| `allergens.openCatalogAll` | Все аллергены |
