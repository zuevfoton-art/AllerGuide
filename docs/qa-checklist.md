...
| 2.2 | Intro: «Пропустить» справа сверху, точки по центру, «Далее» во всю ширину | ☐ | ☐ | ☐ | |
...
| 2.7a | После регистрации на главной — тур coach marks (спотлайт + «Далее») | ☐ | ☐ | ☐ | FR-ONB-07 |
| 2.7b | «Пропустить подсказки» гасит все туры, в т.ч. на других табах | ☐ | ☐ | ☐ | FR-ONB-08 |
| 2.7c | Повторный запуск приложения не показывает подсказки | ☐ | ☐ | ☐ | |
| 2.7d | Второй аккаунт на том же устройстве видит подсказки заново | ☐ | ☐ | ☐ | per-user keys |
...
| 3.6 | Строка профиля целиком открывает редактор; удаление — в `/profile-edit` | ☐ | ☐ | ☐ | |
...
| 4.3 | Wellness: карточка «Состояние сегодня» (бейдж + индекс) и отдельная карточка «Факторы» | ☐ | ☐ | ☐ | нужен интернет |
| 4.4 | «Подробнее» → карта | ☐ | ☐ | ☐ | |
| 4.5 | Блок дневника: quick-add кнопки по профилю (S1: симптомы+питание) | ☐ | ☐ | ☐ | ProfileCapabilities |
| 4.6 | Quick actions: Сканер, Карта | ☐ | ☐ | ☐ | |
| 4.7 | Компактный disclaimer (одна строка + «Подробнее») | ☐ | ☐ | ☐ | `Disclaimer compact` |
| 4.8 | Tab bar: 5 вкладок (Home, Diary, Scanner, Map, SOS); Маркет скрыт без `EXPO_PUBLIC_MARKET=true` | ☐ | ☐ | ☐ | FR-UX-04 |
...
| # | Сценарий | iOS | Android | Web | Примечания |
|---|----------|-----|---------|-----|------------|
| 5.1 | Секция «Записи в дневник» сверху; главная кнопка «Новая запись», ряд Курс/Отчёт; история только с выбранными значениями | ☐ | ☐ | ☐ | FR-DIARY-03/06 |
...
| 5.14 | **§7.3 voice:** микрофон на шагах «Симптомы»/`onset` → текст в поле | ☐ | ☐ | ☐ | FR-DIARY-voice; Chromium web; не на питании/лекарстве |
| 5.15 | **§7.3 voice:** отказ permission / unsupported → wizard остаётся usable | ☐ | ☐ | ☐ | text-fallback |
| 5.16 | Поле симптомов в модалке видно над IME; шапка редактора на экране | ☐ | ☐ | N/A | scroll-to-focus; без `liftStyle` |
...
| # | Сценарий | iOS | Android | Web | Примечания |
|---|----------|-----|---------|-----|------------|
| 7.1 | SOS: профиль + панель 112; ссылка «Все контакты» ведёт в `/sos-edit`, без повторного списка в карточке | ☐ | ☐ | ☐ | |
| 7.1a | Перекрёстные реакции из визарда видны отдельным рядом на SOS (не в `passport.triggers`) | ☐ | ☐ | ☐ | FR-SOS-02, FR-PROF-18 |
...
| 8.5 | Маркет: каталог продуктов | ☐ | ☐ | ☐ | только при `EXPO_PUBLIC_MARKET=true`; иначе `/market` → Главная |
| 8.5a | Две карточки в ряд, фото сверху | ☐ | ☐ | ☐ | узкий экран |
| 8.5b | Фильтры + поиск по снимку | ☐ | ☐ | ☐ | Все / Воздух / … / Аптека |
| 8.5c | CTA Яндекс Маркет / аптека | ☐ | ☐ | ☐ | seed URL офлайн |
| 8.5d | OTC-аптека: disclaimer, без цены | ☐ | ☐ | ☐ | рецептурные скрыты |
| 8.5e | Offline last-good / seed | ☐ | ☐ | ☐ | сеть выключена |
| 8.6 | Disclaimer | ☐ | ☐ | ☐ | |
...
| 10.2 | Напоминание дневника и пыльцы: одно поле времени (`DateTimeField`) | ☐ | ☐ | N/A | push только native |
...
| Backup | `staging-backup-smoke.yaml` | fixture recovery key → upload → banner «Резервная копия отправлена на сервер.» |
