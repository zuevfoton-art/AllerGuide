...
| **Home insights** | `(tabs)/home.tsx` | `home-insights-service`, `wellness-service` | core `home-insights`, `immune-balance`, `wellness*`, `wellness-display` |
...
| **SOS** | `(tabs)/sos.tsx` (read-only); `sos-edit.tsx` из `/profile` | `sos-service`, `sos-passport-service`, `emergency-contact-service`, `medicine-suggest-service` | core `allergy-passport`, `emergency-contacts`, `list-input`, `listProfileAllergenChips` |
...
src/components/       # UI-компоненты (brand/, diary/, onboarding/, profile-setup/, scanner/, map/, therapy/)
...
| `asit-course.tsx` / `prescribed-therapy.tsx` | ASIT / терапия (общий course-editor + OCR) |
...
- **Shell:** `Screen`, `ScreenHeader`, `GlassCard`, `CardTitle`, `Button`, `BrandField`, `BrandPair`, `ActionChip`, `SelectChip`, `SegmentedControl`, `BottomSheet`, `AskFabHost` / `AskChatPanel` / `AskChatSheet`, `Disclaimer`, `Skeleton` (`SkeletonLine` / `SkeletonCard` / `SkeletonBlock`), `EmptyState`, `ErrorBoundary`, `AppLockGate`, `FocusRing`/`SkipLink`, `ListPickerSheet`, …
...
| Diary / home | `diary` (barrel → `diary-schema` + `diary-format`), `diary-stats`, `diary-severity`, `diary-triggers`, `diary-profile`, `diary-reminder`, `diary-wizard-route`, `scan-diary-target`, `voice-diary`, `home-insights`, `immune-balance`, `wellness`, `wellness-display`, `wellness-weights`, `wellness-cross-reactions`, `medicine-catalog` |
