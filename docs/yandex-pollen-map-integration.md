...
**Статус:** гибридный контур A реализован: Яндекс.Карты по локации, live-уровни
Open-Meteo для трёх таксонов, ссылка на карту Яндекс Погоды, кэш и календарный
fallback. Поверх подложки отображаются цветные модельные точки CAMS и список
направлений с низкой нагрузкой; добавлены ольха, полынь и олива. Контур B ожидает
официальный B2B API/tiles Яндекса.
...
Поэтому основной экран использует собственный pollen-overlay: цветные точки
Open-Meteo/CAMS на Яндекс-подложке, легенду и числовые значения. Блок «Пробки»
визуально закрыт pollen-легендой и недоступен для нажатия. Полностью чистая карта
с `controls: []` возможна после настройки ключа JavaScript API Яндекс.Карт;
сам официальный pollen heatmap всё равно требует отдельного B2B API/tiles.
Viewport pollen-overlay фиксирован: cross-origin widget не сообщает приложению
pan/zoom, и интерактивное смещение рассинхронизировало бы модельные точки.
