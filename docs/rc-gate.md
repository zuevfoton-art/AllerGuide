...
| G1c | Design tokens (`fontSize`/`lineHeight` literals, StyleSheet `title` keys, raw hex/rgba) stay on allowlist; new chrome uses `ui.sectionTitle` and `theme.colors.*` | ✅ `check-design-tokens.mjs` | Eng | ✅ wired into `pnpm rc-gate` |
