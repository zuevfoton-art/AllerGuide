...
# Prefer GitHub secrets; then eas.json staging.env; then EAS env:get (preview).
...
read_eas_json_dsn() {
  local key="$1"
  local eas_json="${RESOLVE_STAGING_ERROR_DSN_EAS_JSON:-$ROOT/apps/mobile/eas.json}"
  if [ ! -f "$eas_json" ]; then
    return 0
  fi
  node -e '
    ...
    const env = (eas.build && eas.build.staging && eas.build.staging.env) || {};
    process.stdout.write(String(env[process.argv[2]] || ""));
  ' "$eas_json" "$key"
}

if [ -z "$DSN" ]; then
  try_candidate "$(read_eas_json_dsn EXPO_PUBLIC_ERROR_DSN)" "eas.json:staging.env:EXPO_PUBLIC_ERROR_DSN" || \
    try_candidate "$(read_eas_json_dsn EXPO_PUBLIC_SENTRY_DSN)" "eas.json:staging.env:EXPO_PUBLIC_SENTRY_DSN" || \
    true
fi
...
if [ -n "$DSN" ]; then
  ...
  echo "Set repo secret EXPO_PUBLIC_ERROR_DSN, eas.json staging.env, or EAS preview Sensitive to https://<key>@${EXPECTED_HOST}/<id>."
