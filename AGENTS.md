...
- The web app is already served on `http://localhost:5000` by the `expo-web` terminal (`pnpm --filter mobile exec expo start --web --port 5000`); read its logs or restart it there. `pnpm start` from the root runs `expo start` without forcing web/port.
