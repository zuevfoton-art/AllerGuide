...
- Package manager is **pnpm** (`packageManager: pnpm@10.34.4`). The Cloud Agent environment runs `pnpm install --frozen-lockfile` from the repo root on checkout, so dependencies are ready; re-run it yourself only after changing the lockfile (or if `node_modules` is missing).
- The environment starts a local **Postgres 16** and writes `~/.allerguide-cloud.env`. `source ~/.allerguide-cloud.env` exports the same `DATABASE_URL`/`DIRECT_DATABASE_URL`/`DB_SSL`/`JWT_SECRET`/flag values the `api-integration` CI job uses, against `allerguide:allerguide@127.0.0.1:5432` (databases `allerguide_test` and `allerguide`). If the file is absent, provision Postgres yourself.
- `ANDROID_HOME` is preset and JDK 17 is installed, so `apps/mobile/android` Gradle builds work without setup. The Android **emulator and system images are deliberately absent**: nested KVM does not work in Cloud Agent VMs (`emulator` hangs at 0% CPU and never appears in `adb devices`), so Maestro flows cannot run here — they run on GitHub Actions. Reinstall with `sdkmanager` if that ever changes.

### Mobile app (the product) — `apps/mobile`
- Run the web app: `pnpm --filter mobile exec expo start --web --port 5000` (serves on `http://localhost:5000`; works from any directory). If the environment defines an `expo-web` terminal, the server is already running there — read its logs or restart it instead of starting a second one, because a live server makes the next `expo start` fail with «Port 5000 is running this app in another window» and then exit for want of input. `pnpm start` from the root runs `expo start` without forcing web/port.
...
- Gotcha: the dev server regenerates `apps/mobile/.expo/types/router.d.ts`, and the route order it emits is not stable, so the file shows up modified even when no route changed. It is tracked (despite `.expo/` being gitignored) because `apps/mobile/tsconfig.json` feeds it to `pnpm typecheck`, so run `git checkout -- apps/mobile/.expo/types/router.d.ts` before committing unless you actually added or removed a route.
...
- The Cloud Agent environment provides the Postgres instance the API needs. Reproduce the `api-integration` CI job with `source ~/.allerguide-cloud.env && pnpm --filter api db:migrate && pnpm --filter api test:integration`. Outside Cloud, provision Postgres yourself and set `DATABASE_URL`.
...
- Ask chat (`src/routes/ask.ts`): explainer via `/api/ask`. Enable with `AI_CHAT_ENABLED=true` + `AI_SCAN_ENABLED`; mobile `EXPO_PUBLIC_AI_CHAT=true` (default on for local + staging). Distress wording returns `{ handoff: 'sos' }` without calling the model.
