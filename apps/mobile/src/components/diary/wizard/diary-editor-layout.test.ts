...
describe('splitDiaryScreenForIme', () => {
  ...
  it('pins itching chips and keeps skin text fields in the scroll', () => {
  ...
  it('pins itching on the real skin grouped screen and scrolls the text fields', () => {
  ...
  it('does not pin two medicine text fields (they need the scroll to swap under Gboard)', () => {
    ...
    expect(splitDiaryScreenForIme([medicine, dosage, takenAt])).toEqual({
      pinnedSteps: [],
      scrolledSteps: [medicine, dosage, takenAt],
    });
  });

  it('still leaves room for Gboard padding after pinning compact chips only', () => {
    ...
    expect(
      diaryEditorSheetMaxHeight(2400) - pad - (64 + 120) - 143,
